...
NS_INLINE BOOL FXEqualIndexSection(FXIndexSection indexSection1,FXIndexSection indexSection2) {
    return indexSection1.index == indexSection2.index && indexSection1.section == indexSection2.section;
}

NS_INLINE FXIndexSection FXMakeIndexSection(NSInteger index, NSInteger section) {
    FXIndexSection indexSection;
    indexSection.index = index;
    indexSection.section = section;
    return indexSection;
}

@interface FXCyclePagerView () <UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, FXCyclePagerTransformLayoutDelegate> {
    struct {
...
@implementation FXCyclePagerView
...
- (void)setDelegate:(id<FXCyclePagerViewDelegate>)delegate {
    ...
    if (self.collectionView && self.collectionView.collectionViewLayout) {
        ((FXCyclePagerTransformLayout *)self.collectionView.collectionViewLayout).delegate = _delegateFlags.applyTransformToAttributes ? self : nil;
    }
}

- (void)setDataSource:(id<FXCyclePagerViewDataSource>)dataSource {
...
- (void)scrollToNearlyIndexAtDirection:(FXPagerScrollDirection)direction animate:(BOOL)animate {
    FXIndexSection indexSection = [self nearlyIndexPathAtDirection:direction];
...
- (void)scrollToItemAtIndex:(NSInteger)index animate:(BOOL)animate {
    ...
    [self scrollToItemAtIndexSection:FXMakeIndexSection(index, index >= self.curIndex ? _indexSection.section : _indexSection.section+1) animate:YES];
}

- (void)scrollToItemAtIndexSection:(FXIndexSection)indexSection animate:(BOOL)animate {
...
- (void)updateLayout {
    ...
    ((FXCyclePagerTransformLayout *)_collectionView.collectionViewLayout).layout = self.layout;
}
...
- (BOOL)isValidIndexSection:(FXIndexSection)indexSection {
...
- (FXIndexSection)nearlyIndexPathAtDirection:(FXPagerScrollDirection)direction{
    return [self nearlyIndexPathForIndexSection:_indexSection direction:direction];
}

- (FXIndexSection)nearlyIndexPathForIndexSection:(FXIndexSection)indexSection direction:(FXPagerScrollDirection)direction {
    if (indexSection.index < 0 || indexSection.index >= _numberOfItems) {
        return indexSection;
    }
    
    if (!_isInfiniteLoop) {
        if (direction == FXPagerScrollDirectionRight && indexSection.index == _numberOfItems - 1) {
            return _autoScrollInterval > 0 ? FXMakeIndexSection(0, 0) : indexSection;
        } else if (direction == FXPagerScrollDirectionRight) {
            return FXMakeIndexSection(indexSection.index+1, 0);
        }
        
        if (indexSection.index == 0) {
            return _autoScrollInterval > 0 ? FXMakeIndexSection(_numberOfItems - 1, 0) : indexSection;
        }
        return FXMakeIndexSection(indexSection.index-1, 0);
    }
    
    if (direction == FXPagerScrollDirectionRight) {
        if (indexSection.index < _numberOfItems-1) {
            return FXMakeIndexSection(indexSection.index+1, indexSection.section);
        }
        if (indexSection.section >= kPagerViewMaxSectionCount-1) {
            return FXMakeIndexSection(indexSection.index, kPagerViewMaxSectionCount-1);
        }
        return FXMakeIndexSection(0, indexSection.section+1);
    }
    ...
    if (indexSection.section <= 0) {
        return FXMakeIndexSection(indexSection.index, 0);
    }
    return FXMakeIndexSection(_numberOfItems-1, indexSection.section-1);
}

- (FXIndexSection)caculateIndexSectionWithOffsetX:(CGFloat)offsetX {
    if (_numberOfItems <= 0) {
        return FXMakeIndexSection(0, 0);
    }
    ...
    return FXMakeIndexSection(curIndex, curSection);
}

- (CGFloat)caculateOffsetXAtIndexSection:(FXIndexSection)indexSection{
    if (_numberOfItems == 0) {
        return 0;
    }
    UICollectionViewFlowLayout *layout = (UICollectionViewFlowLayout *)_collectionView.collectionViewLayout;
...
- (void)resetPagerViewAtIndex:(NSInteger)index {
    ...
    [self scrollToItemAtIndexSection:FXMakeIndexSection(index, _isInfiniteLoop ? kPagerViewMaxSectionCount/3 : 0) animate:NO];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    FXIndexSection indexSection = _indexSection;
    ...
    if (_delegateFlags.didScrollFromIndexToNewIndex && !FXEqualIndexSection(_indexSection, indexSection)) {
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelociFX:(CGPoint)velociFX targetContentOffset:(inout CGPoint *)targetContentOffset {
    ...
    FXPagerScrollDirection direction = FXPagerScrollDirectionRight;
    if ((scrollView.contentOffset.x < 0 && targetContentOffset->x <= 0) || (targetContentOffset->x < scrollView.contentOffset.x && scrollView.contentOffset.x < scrollView.contentSize.width - scrollView.frame.size.width)) {
        direction = FXPagerScrollDirectionLeft;
    }
    FXIndexSection indexSection = [self nearlyIndexPathForIndexSection:_indexSection direction:direction];
...
#pragma mark - FXCyclePagerTransformLayoutDelegate
...
- (void)dealloc {
    ((FXCyclePagerTransformLayout *)_collectionView.collectionViewLayout).delegate = nil;
