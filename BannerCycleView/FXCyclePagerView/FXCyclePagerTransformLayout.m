...
typedef NS_ENUM(NSUInteger, FXTransformLayoutItemDirection) {
    FXTransformLayoutItemLeft,
    FXTransformLayoutItemCenter,
    FXTransformLayoutItemRight,
};


@interface FXCyclePagerTransformLayout () {
    struct {
...
@interface FXCyclePagerViewLayout ()
...
- (void)setLayout:(FXCyclePagerViewLayout *)layout {
    _layout = layout;
...
- (FXTransformLayoutItemDirection)directionWithCenterX:(CGFloat)centerX {
    FXTransformLayoutItemDirection direction= FXTransformLayoutItemRight;
...
- (void)initializeTransformAttributes:(UICollectionViewLayoutAttributes *)attributes layoutType:(FXCyclePagerTransformLayoutType)layoutType {
    switch (layoutType) {
        case FXCyclePagerTransformLayoutLinear:
            [self applyLinearTransformToAttributes:attributes scale:_layout.minimumScale alpha:_layout.minimumAlpha];
            break;
        case FXCyclePagerTransformLayoutCoverflow:
        {
...
- (void)applyTransformToAttributes:(UICollectionViewLayoutAttributes *)attributes layoutType:(FXCyclePagerTransformLayoutType)layoutType {
    switch (layoutType) {
        case FXCyclePagerTransformLayoutLinear:
...
- (void)applyLinearTransformToAttributes:(UICollectionViewLayoutAttributes *)attributes scale:(CGFloat)scale alpha:(CGFloat)alpha {
    ...
    if (_layout.adjustSpacingWhenScroling) {
        FXTransformLayoutItemDirection direction = [self directionWithCenterX:attributes.center.x];
        CGFloat translate = 0;
        switch (direction) {
            case FXTransformLayoutItemLeft:
                translate = 1.15 * attributes.size.width*(1-scale)/2;
                break;
            case FXTransformLayoutItemRight:
                translate = -1.15 * attributes.size.width*(1-scale)/2;
                break;
            default:
                // center
...
@implementation FXCyclePagerViewLayout
